...
let rec est_compatible t1 t2 =
  ...
  | Pointeur (p1), Pointeur (p2) -> if p1 = Undefined || p2 = Undefined then true else est_compatible p1 p2
  | Pointeur (p1), p2 -> est_compatible p1 p2
  | p1, Pointeur (p2) -> est_compatible p1 p2
  | _ -> false 
