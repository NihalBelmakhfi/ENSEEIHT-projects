...
public class MoniteurImpl extends UnicastRemoteObject implements Moniteur {
     ...
     * RMI *
     *******/
    private static String serverURL; // server site address
    private static int port = Registry.REGISTRY_PORT; // server site port
    ...
    public static void init() {
        try {
            ...
            serverURL = "//" + InetAddress.getLocalHost().getHostName() + ":" + port + "/Server";
    ...
    public void feuVert(String site, int facteur) throws RemoteException {
        ...
        try {
            for (int i = 0; i < facteur; i++) {
                ...
                if (d == -2)
                    Thread.sleep(1_800_000_000); // une implémentation de l'infini
                else
                    Thread.sleep(d * 1000);
            }
        ...
        actions.put("delai", new Action() {
            public void run(String[] args) {
                ...
                if (name.equals("*")) {
                    try {
                        for (Client_itf s : sites) {
                            if (délais.get(s.getSite()) > -2)
                                délais.put(s.getSite(), d);
                        }
                    } catch (RemoteException rex) {
                        System.out.println("erreur accès client : " + rex.getMessage());
                        rex.printStackTrace();
                    ...
                    if (délais.keySet().contains(name)) {
                        if (délais.get(name) > -2)
                            délais.put(name, d);
                        else
                            System.out.println("??? (site en panne)");
                    } else
                        System.out.println("??? (nom de site inconnu)");
                }
            }
        });
