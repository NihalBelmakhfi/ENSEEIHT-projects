...
public class Interpolateur : MonoBehaviour
{
    ...
    (List<float>, List<float>) buildParametrisationReguliere(int nbElem, float pas)
    {
        ...
        float tpsCourant = 0.0f;
        for(int i = 0; i < T.Count-1; i++){
            tpsCourant = T[i];
            while (tpsCourant < T[i+1]){
                tToEval.Add(tpsCourant);
                tpsCourant += pas;
            }
        }
    ...
    (List<float>, List<float>) buildParametrisationDistance(int nbElem, float pas)
    {
        ...
        for (int i = 0 ; i < nbElem - 1; i++) {
            dist += (float) Mathf.Sqrt(Mathf.Pow(X[i+1] - X[i], 2) + Mathf.Pow(Y[i+1] - Y[i], 2));
            T.Add(dist); 
        }

        // Construction des échantillons
        // On initialise tToEval avec la distance parcourue jusqu'aupremier pt
        float tpsCourant = dist;
        for (int i = 0; i < T.Count - 1; i++) {
            tpsCourant = T[i];
            while (tpsCourant < T[i+1]) {
                tToEval.Add(tpsCourant);
    ...
    (List<float>, List<float>) buildParametrisationRacineDistance(int nbElem, float pas)
    {
        ...
        T.Add(dist);
        for (int i = 0 ; i < nbElem - 1; i++) {
            dist += (float) Mathf.Sqrt(Mathf.Sqrt(Mathf.Pow(X[i+1] - X[i], 2) + Mathf.Pow(Y[i+1] - Y[i], 2)));
            T.Add(dist); 
        }
