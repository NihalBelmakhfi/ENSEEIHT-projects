...
struct
  ...
  let rec string_of_affectable a =
    match a with
      |Ident s -> s
      |Dereferencement p -> string_of_affectable p
  ...
  let rec string_of_expression e =
    ...
    | ConditionnelleTer (e1,e2,e3) -> "(" ^ (string_of_expression e1) ^
  ...
  let rec string_of_instruction i =
    ...
    | ConditionnelleOpt (c,t) ->  "Conditionnelle  : IF "^(string_of_expression c)^"\n"^
                                  "THEN \n"^((List.fold_right (fun i tq -> (string_of_instruction i)^tq) t ""))^"\n"
